...
// ===== Google Apps Script Web App URL (สำหรับส่งอีเมลแจ้งสถานะ) =====
const GAS_EMAIL_URL = "https://script.google.com/macros/s/AKfycbw2na_npXMXQMaawUbS_fr4udtCGcJwNybgbKQSC8G57RRe6aGOshfTxyUl6Itu3VCtSA/exec";
...
function setupAdminEventListeners() {
    ...
    if (searchInput) searchInput.addEventListener('input', () => { loadComplaintsTable(); renderActiveFilterChips(); });
    if (categoryFilter) categoryFilter.addEventListener('change', () => { loadComplaintsTable(); renderActiveFilterChips(); });
    if (statusFilter) statusFilter.addEventListener('change', () => { loadComplaintsTable(); renderActiveFilterChips(); });
    if (timeFilter) timeFilter.addEventListener('change', () => { loadComplaintsTable(); renderActiveFilterChips(); });
    if (sortFilter) sortFilter.addEventListener('change', () => loadComplaintsTable());
...
async function loadStatistics() {
    try {
        ...
        snapshot.forEach(doc => {
            ...
            // สะสมคะแนนความพึงพอใจ
            if (data.feedback && typeof data.feedback.rating === 'number') {
                totalSatisfaction += data.feedback.rating;
                satisfactionCount++;
            }
        ...
        const avgRatingBox = document.getElementById('avgRatingBox');
        if (avgRatingBox) {
            if (satisfactionCount > 0) {
                const avgRating = totalSatisfaction / satisfactionCount;
                avgRatingBox.innerHTML = `${avgRating.toFixed(1)} <i class="bi bi-star-fill text-warning fs-5 mx-1"></i> <small class="text-muted fw-normal" style="font-size: 0.75rem;">(${satisfactionCount} รายการ)</small>`;
            } else {
                avgRatingBox.textContent = 'ไม่มีข้อมูล';
            }
        }
...
// ===== ฟังก์ชันแสดง Active Filter Chips =====
function renderActiveFilterChips() {
    ...
    const statusLabels = { waiting: 'รอรับเรื่อง', accepted: 'รับเรื่องแล้ว', 'in-progress': 'ดำเนินการ', resolved: 'เสร็จสิ้น', rejected: 'ไม่รับเรื่อง' };
    const timeLabels = { daily: 'วันนี้', monthly: 'เดือนนี้', yearly: 'ปีนี้' };

    let chips = [];
    if (searchTerm) chips.push({ label: `🔍 "${searchTerm}"`, clearFn: () => { document.getElementById('searchInput').value = ''; loadComplaintsTable(); renderActiveFilterChips(); } });
    if (categoryFilter) chips.push({ label: `📁 ${categoryFilter}`, clearFn: () => { document.getElementById('categoryFilter').value = ''; loadComplaintsTable(); renderActiveFilterChips(); } });
    if (statusFilter) chips.push({ label: `📌 ${statusLabels[statusFilter] || statusFilter}`, clearFn: () => { document.getElementById('statusFilter').value = ''; loadComplaintsTable(); renderActiveFilterChips(); } });
    if (timeFilter) chips.push({ label: `📅 ${timeLabels[timeFilter] || timeFilter}`, clearFn: () => { document.getElementById('timeFilter').value = ''; loadComplaintsTable(); renderActiveFilterChips(); } });
    ...
    container.innerHTML = chips.map((chip, i) =>
        `<span class="active-filter-chip" id="filterChip${i}">${chip.label}<button class="chip-close-btn" onclick="(${chip.clearFn.toString()})()" aria-label="ลบตัวกรอง">✕</button></span>`
    ).join('');
}
    ...
    unsubscribe = complaintsCollection
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // ค้นหาขยายครอบคลุมหลายฟิลด์
                const matchesSearch = !searchTerm ||
                    (data.ticketId && data.ticketId.toLowerCase().includes(searchTerm)) ||
                    (data.title && data.title.toLowerCase().includes(searchTerm)) ||
                    (data.reporterName && data.reporterName.toLowerCase().includes(searchTerm)) ||
                    (data.reporterEmail && data.reporterEmail.toLowerCase().includes(searchTerm)) ||
            ...
            // เรียงลำดับตาม sortFilter
            const statusOrder = { waiting: 0, pending: 0, accepted: 1, 'in-progress': 2, resolved: 3, rejected: 4 };
            if (sortFilter === 'oldest') {
                ...
                filteredDocs.sort((a, b) => (statusOrder[a.status] ?? 9) - (statusOrder[b.status] ?? 9));
            }
            // newest = default (Firestore orderBy desc ไว้อยู่แล้ว)

            // แสดงจำนวนผลลัพธ์
            const resultCountEl = document.getElementById('resultCount');
            const totalCountEl = document.getElementById('totalCount');
            if (resultCountEl) resultCountEl.textContent = filteredDocs.length;
                    ...
                    <tr>
                        <td colspan="9" class="text-center text-muted py-5">
                            <div class="py-3">
                                ...
                                <div class="mt-2 fw-bold">ไม่พบข้อมูลที่ตรงตามเงื่อนไข</div>
                                <div class="small mt-1">ลองปรับตัวกรองหรือ<button class="btn btn-link btn-sm p-0 ms-1" onclick="clearAllFilters()">ล้างตัวกรองทั้งหมด</button></div>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            <div class="text-start admin-detail-modal">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-7">
                        ...
                        ${data.feedback ? `
                        <div class="detail-section mt-4 p-3 rounded-4 bg-warning bg-opacity-10 border border-warning-subtle">
                            <h6 class="text-warning-emphasis fw-bold mb-2 small text-uppercase"><i class="bi bi-star-fill me-2 text-warning"></i>ผลการประเมินจากผู้แจ้ง</h6>
                            <div class="d-flex align-items-center mb-1">
                                <div class="me-2 fs-5">
                                    ${Array.from({length: data.feedback.rating || 0}, () => '<i class="bi bi-star-fill text-warning mx-0.5"></i>').join('')}
                                    ${Array.from({length: 5 - (data.feedback.rating || 0)}, () => '<i class="bi bi-star text-muted mx-0.5"></i>').join('')}
                                </div>
                                <span class="fw-bold text-dark">${data.feedback.rating} / 5</span>
                            </div>
                            ${data.feedback.comment ? `<p class="mb-0 text-muted small mt-2 bg-white p-2 rounded-3 border">"${data.feedback.comment}"</p>` : ''}
                        </div>
                        ` : ''}
                    </div>
...
// ===== ฟังก์ชันส่งอีเมลแจ้งสถานะผ่าน Google Apps Script =====
async function sendStatusEmailNotification(data, newStatus, adminMessage) {
    if (!GAS_EMAIL_URL || !data.reporterEmail) return;

    const statusTextTh = getStatusText(newStatus);
    ...
    const payload = {
        ...
        to_name: data.reporterName || 'ผู้แจ้ง',
        ticket_id: data.ticketId,
        title: data.title,
        category: data.category,
        new_status: statusTextTh,
        status_emoji: statusEmoji,
        admin_message: adminMessage || 'ไม่มีข้อความเพิ่มเติม',
    ...
    try {
        ...
        console.log(`✅ STATUS EMAIL SENT to ${data.reporterEmail} (Status: ${statusTextTh})`);
    } catch (error) {
        console.error('❌ Status email failed:', error);
        // ไม่ block workflow ถ้าส่งอีเมลไม่สำเร็จ
    }
}
